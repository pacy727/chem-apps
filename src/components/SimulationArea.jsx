...
useEffect(() => {
  ...
  const simulationStep = () => {
    ...
    const updatedAtoms = atomsAfterBonds.map(atom => 
      updateAtomPosition(atom, dimensions.width, dimensions.height, atomsAfterBonds, bonds)
    );
  ...
  simulationAtoms.forEach(atom => {
    atom.bonds.forEach(bond => {
      ...
      if (bond.type === 'covalent') {
        // 共有結合
        drawCovalentBond(ctx, atom, partnerAtom, bond);
...
// 共有結合の描画
const drawCovalentBond = (ctx, atom1, atom2, bond) => {
  const x1 = atom1.x;
  const y1 = atom1.y;
  const x2 = atom2.x;
  const y2 = atom2.y;
  ...
  // 電子位置が明示されている場合はその位置を使用
  if (electronPos1 !== undefined && electronPos2 !== undefined) {
    // 原子1側の電子位置
    const e1Radius = atom1.element.atomicRadius * 0.9;
    e1x = atom1.x + Math.cos(electronPositions[electronPos1].angle) * e1Radius;
    e1y = atom1.y + Math.sin(electronPositions[electronPos1].angle) * e1Radius;
    
    // 原子2側の電子位置
    const e2Radius = atom2.element.atomicRadius * 0.9;
    e2x = atom2.x + Math.cos(electronPositions[electronPos2].angle) * e2Radius;
    e2y = atom2.y + Math.sin(electronPositions[electronPos2].angle) * e2Radius;
  } else {
    // 位置が明示されていない場合は中間点を使用
    e1x = (x1 + x2) / 2;
    e1y = (y1 + y2) / 2;
    e2x = e1x;
    e2y = e1y;
  ...
  const radius = Math.min(distance(x1, y1, x2, y2) / 2, 
